...
Durante una noche en un restaurant pasan varios grupos de diversa cantidad de personas. 
Para llevar control de esto, el dueño va anotando en su libreta cuánta gente entra y sale. 
Para hacerlo rápido decide que la mejor forma de llevarlo adelante es escribir un número al lado del otro, 
usando números positivos para los grupos que entran y negativos para los que salen. Gracias a estas anotaciones el dueño es capaz de hacer análisis del flujo de clientes. 
Por ejemplo, le interesa saber en qué momento de la noche superó una determinada cantidad de clientes totales que ingresaron 
(sin importar cuántos hay en el momento en el local).

Implementar la función pos_umbral, que dada una secuencia de enteros (puede haber negativos) devuelve la posición en la cual se supera el valor de umbral, 
teniendo en cuenta sólo los elementos positivos. Se debe devolver -1 si el umbral no se supera en ningún momento
...
def pos_umbral(s:list[int], u:int)->int:
    ...
    while i < len(s):
        if s[i] > 0:
            contador_de_personas += s[i]
            if contador_de_personas > u:
                return i
            i +=1
        i +=1
    return res        
#print(pos_umbral([5,-2,1,4,-7,0],5))
        

"""
3) Columnas repetidas [3 puntos]
Implementar la función columnas_repetidas, que dada una matriz no vacía de m columnas (con m par y m ≥ 2) 
devuelve True si las primeras m/2 columnas son iguales que las últimas m/2 columnas. Definimos a una secuencia de secuencias como matriz si 
todos los elementos de la primera secuencia tienen la misma longitud.
...
"""
def primera_mitad(s:list[int])->list[int]:
    res = []
    for i in range(int(len(s)/2)):
        res.append(s[i])
    return res
def segunda_mitad(s:list[int])->list[int]:
    res = []
    for i in range(int(len(s)/2),len(s)):
        res.append(s[i])
    return res
def columnas_repetidas(mat:list[int])->bool:
    for i in mat:
        if not ((primera_mitad(i)) == segunda_mitad(i)):
            return False
    return True
#print(columnas_repetidas([[1,  3, 1, 3],[0, 6, 0,  6],[0,  1,  0,  1]]))



"""
4) Rugby 4 naciones [3 puntos]
Desde hace más de 10 años existe en el mundo del rugby un torneo que disputan anualmente 4 selecciones del sur global (Argentina, Australia, Nueva Zelanda y Sudáfrica). 
Este torneo se llama "The rugby championship" o comunmente "4 naciones", ya que suplantó al viejo "3 naciones".

Implementar la función cuenta_posiciones_por_nacion que dada la lista de naciones que compiten en el torneo, y el diccionario que tiene 
los resultados de los torneos anuales 

año : [posiciones_naciones]
posiciones_naciones es una lista de strings con los nombres de las naciones

genere un diccionario de naciones:#posiciones, que para cada Nación devuelva la lista de cuántas veces salió en esa posición.
...
def cuenta_posiciones_por_nacion(naciones:list[int], torneos:dict[int,list[str]])->dict[str,list[str]]:
    res:dict = {}
    for i in naciones:
        if not i in res.keys():
            lista_de_ceros = [0]*len(naciones)
            res[i] = lista_de_ceros
